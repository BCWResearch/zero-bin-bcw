...
{
    provider
        .get_block(target_block_id, full_transaction_data.into())
        .await?
    ...
    TransportT: Transport + Clone,
{
    ...
    let target_block = provider
        .get_block(target_block_id, BLOCK_WITH_FULL_TRANSACTIONS.into())
        .await?
    ...
    futures::stream::iter(
        prev_hashes
            ...
            .map(|(dst, n)| {
                ...
                async move {
                    let block = provider
                        .get_block(n.into(), BLOCK_WITH_FULL_TRANSACTIONS.into())
                        .await
